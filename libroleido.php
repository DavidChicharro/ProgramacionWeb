<!DOCTYPE html>
	...
	<body onload="contador()">
		...
		<section class="opinion-valoracion">
			...
			<form name="modificar-libro" action="opinion_libro.php" method="post" onkeyup="formModifOpinValidation(this)">
				<fieldset>	
					<?php
					require_once ('classes/valoraciones.class.inc');

					$valoracion = new Valoraciones();
					$valoracionUsuario = $valoracion->obtenerOpinionUsuario( $_SESSION["id"], $isbnLibro );

					function radioSelected($val, $checked){
						if($val == $checked)
							return "checked";
						else
							return "";
					...
					echo ('							
						...
						<textarea id="opinion" name="opinion" rows="6" cols="80" maxlength="500" required onkeyup="contador()">'.$valoracionUsuario['opinion'].'</textarea><br>
						<span id="current">0</span><span>/ 500</span>

						<input type="text" id="id_valoracion" name="id_valoracion" hidden required value="'.$valoracionUsuario['id_valoracion'].'">
					...
					for ($i=1; $i<=5 ; $i++) {
						echo ('<input type="radio" id="valoracion'.$i.'" name="valoracion" value="'.$i.'" '.radioSelected($i, $valoracionUsuario['nota']).' onclick="radioLeidoValidation(this)" required/>
					...
					<input class="btn-disabled" type="submit" id="btnEnviar" name="modificar" value="Modificar" disabled>
				</fieldset>
			</form>
		</section>
